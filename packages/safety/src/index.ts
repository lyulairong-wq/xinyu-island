...
const blockedRules: ReadonlyArray<BlockedRule> = [
  ...
  },
  {
    ...
    pattern: /(?:根据.*直接诊断(?:疾病|我)|(?:该不该|是否|能否).*(?:自行)?(?:停药|停用药物|更改用药)|(?:替我|给我|帮我|告诉我|怎么|如何|能否|请|制定|决定|写).*(?:诊断|处方药.*(?:剂量|次数)|用药剂量|治疗方案|起诉步骤|保证收益|贷款.*(?:审批规避|规避办法)|投资决定|规避.*法律责任|吃什么药|股票内幕|投资建议)|诊断结论\s*[:：]|(?:用药剂量|治疗方案|起诉步骤|保证收益(?:方案|承诺)|贷款审批规避(?:方法|办法)|投资决定)\s*[:：]|规避法律责任的(?:方法|步骤)\s*[:：]|真实资产.*投资)/i,
    standaloneSafetyPatterns: [
      /^(?:我|本助手|助手|AI)?(?:不能|不会|无法|拒绝)(?:替你|为你|向你)?(?:回答如何)?(?:提供|给出|根据|说明|制定|决定|作出|诊断|建议)?(?:疾病诊断|诊断疾病|诊断结论|个人治疗方案|治疗方案|治疗方法|治疗决定|停药决定|是否(?:自行)?停药|自行停药|处方药(?:剂量|次数)?|用药剂量|起诉步骤|保证收益(?:方案|承诺)?|贷款(?:审批规避(?:方法|办法)?)?|投资决定|规避法律责任(?:的方法|步骤)?|吃什么药|股票内幕|投资建议)[。.!！?？]?$/i,
      /^(?:替用户作出|提供|给出|根据|说明|制定|决定|作出|诊断|建议)?(?:疾病诊断|诊断疾病|诊断结论|个人治疗方案|治疗方案|治疗方法|治疗决定|停药决定|是否(?:自行)?停药|自行停药|处方药(?:剂量|次数)?|用药剂量|起诉步骤|保证收益(?:方案|承诺)?|贷款(?:审批规避(?:方法|办法)?)?|投资决定|规避法律责任(?:的方法|步骤)?|吃什么药|股票内幕|投资建议)(?:是|非常)?(?:不安全|危险|违法|不当|不可接受|不应该)(?:的)?[。.!！?？]?$/i,
      /^(?:我)?(?:谴责|反对|批判|不赞成)(?:替用户作出|提供|给出|根据|说明|制定|决定|作出|诊断|建议)?(?:疾病诊断|诊断疾病|诊断结论|个人治疗方案|治疗方案|治疗方法|治疗决定|停药决定|是否(?:自行)?停药|自行停药|处方药(?:剂量|次数)?|用药剂量|起诉步骤|保证收益(?:方案|承诺)?|贷款(?:审批规避(?:方法|办法)?)?|投资决定|规避法律责任(?:的方法|步骤)?|吃什么药|股票内幕|投资建议)(?:行为)?[。.!！]?$/i
    ],
    topicPattern: /(?:诊断|处方药|用药剂量|停药|停用药物|更改用药|治疗方案|治疗方法|治疗决定|起诉步骤|保证收益|贷款|投资决定|法律责任|吃什么药|股票内幕|投资建议|真实资产)/i,
    unsafeInstructionPattern: /(?:该不该|是否|替我|给我|帮我|告诉我|怎么|如何|能否|请|制定|决定|写|说明|列出|接下来|接着|随后|然后|现在|实际).*(?:诊断|处方药|用药剂量|停药|停用药物|更改用药|治疗方案|起诉|保证收益|贷款|投资|法律责任|可执行方案|SAMPLEDOSE)|SAMPLEDOSE|syntheticactionable/i
  }
];
...
function isStandaloneEnglishSafetyStatement(text: string): boolean {
  return /^(?:(?:i|we|this assistant)\s+(?:cannot|can not|can't|will not|won't|do not|don't|refuse to)\s+(?:provide|give|share|reveal|disclose|show|offer|write|explain|answer)\b[^:;!?]*[.!]?)$/i.test(text);
}
