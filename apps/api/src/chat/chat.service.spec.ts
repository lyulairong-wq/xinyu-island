...
describe("ChatService", () => {
  it("creates an official conversation and returns a free mock reply", () => {
    const service = new ChatService();
    const conversation = service.createConversation("user-1", "lin");
    const result = service.sendMessage("user-1", conversation.id, { content: "你好", mode: "free", memoryEnabled: false });
