...
- 入口 UI 分别展示并要求选择 `terms`、`privacy`、`entertainment_notice` 三项，不提供合并或默认勾选。
- 每项均标记为“内部封闭测试版 v1.0”，仅用于当前封闭测试准入，不构成已定稿的用户协议、隐私政策或法律文本。
- 注册按钮在三项全部选择前保持禁用；客户端仅在完整选择后提交按固定顺序生成的 `type` 与版本 `1.0` 记录。
- 服务端仍是最终准入与持久化审计的权威，客户端限制不替代服务端的三项同意校验。
